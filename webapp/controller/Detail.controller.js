sap.ui.define([
...
], function (BaseController, JSONModel, formatter, mobileLibrary, Filter, FilterOperator, Device, Dialog, Button, Label, Text, DialogType, MessageBox, Fragment, Sorter, Export, ExportTypeCSV, library, Spreadsheet) {
  "use strict";
  ...
  return BaseController.extend("hayat.hcm.orgchart.md.controller.Detail", {
    ...
    onInit: function () {
      ...
      this.bus = sap.ui.getCore().getEventBus();
      this.bus.subscribe("Master", "refreshDetailPage", this._focusNode, this);

      // this.bus.subscribe("DetailDetail", "refreshDetailPage", this._onVariantSelect, this); // ??
      this.bus.subscribe("DetailDetailVariant", "refreshDetailPage", this._onVariantSelect, this); // ??
      ...
      $(window).resize(function () {
        ...
        if (thiz.chart)
          setTimeout(thiz.chart.setLayout(OrgChart.tree), 1000);
    ...
    _handleVariantSavePress: function (oEvent) {
      ...
      this.getModel().callFunction("/addVariant", {
        ...
        success: $.proxy(function (oData, oResponse) {
          ...
          this.bus.publish("DetailDetailVariant", "refreshDetailPage", { "Variant": oData.addVariant.MessageV4 });
          this.bus.publish("Detail", "refreshDetailPage", { "Variant": oData.addVariant.MessageV4 });
    ...
    fullScreen: function (oEvent) {
      ...
      if (this.chart)
        setTimeout(this.chart.setLayout(OrgChart.tree), 2000); //???
 ...
 */
    ...
    _setGraph: function (IVariant) {
      ...
      if (this._type === '12') {
        ...
        _theme = "olivia";
        if (this.byId("graphTheme") && this.byId("graphTheme").getSelectedKey()) {
      ...
      this.getOwnerComponent().getModel().callFunction("/getC", {
        urlParameters: _d,
        success: $.proxy(function (oData, oResponse) {
          ...
          if (oData.results.length > 0) {
            ...
            nodes = nodes.sort(
              (p1, p2) => (p1.H < p2.H) ? 1 : (p1.H > p2.H) ? -1 : 0);
            ...
            for (var y = 0; y < nodes.length; y++) {
              ...
              nodes[y].Ph = "/sap/opu/odata/sap/HCM_EMPLOYEE_LOOKUP_SRV/EmployeeInfoSet('" + nodes[y].Pe + "')/$value";

              if (document.baseURI.indexOf("mobil.hayat.com.tr") !== -1) {
                nodes[y].Phx = "http://192.168.2.154:8000/sap/opu/odata/sap/ZHR_PROJ_026_SRV/EmployeeInfoSet('" + nodes[y].Pe + "')/$value?saml2=disabled";
              } else {
                nodes[y].Phx = "http://192.168.2.194:8000/sap/opu/odata/sap/ZHR_PROJ_026_SRV/EmployeeInfoSet('" + nodes[y].Pe + "')/$value?saml2=disabled";
              }
            }
              ...
              if (this._type === '2') {
                ...
                if (nodes[i].O === 'O' || nodes[i].O === 'S') {
                  for (var x = 0; x < nodes.length; x++) {
                    if (nodes[x].O === 'O' && nodes[x].id === nodes[i].pid) {
                      ...
                      nodes[x].Tvplans = parseInt(nodes[x].Tvplans) + parseInt(nodes[i].V);
                      nodes[x].Txplans = parseInt(nodes[x].Txplans) + parseInt(nodes[i].X);
                      nodes[x].Tfplans = parseInt(nodes[x].Tfplans) + parseInt(nodes[i].F);
            ...
            var chart = this.chart = new OrgChart(document.getElementById("orgchart"), {
              ...
              menu: {
                ...
                'focus-mode': {
                  icon: function () { var checked = thiz.focusMode == true ? 'checked' : ''; return '<input id="focus-mode" class="check-box" onclick="sap.ui.getCore().byId(\'' + thiz.getView().getId() + '\').getController().fmodeclick()" type="checkbox" ' + checked + '>'; },
                  text: '<label for="focus-mode">Odakla</label>',
                  onClick: function () { return false; }
                },
                'scale-mode': {
                  icon: function () { var checked = thiz.scaleMode == true ? 'checked' : ''; return '<input id="focus-mode" class="check-box" onclick="sap.ui.getCore().byId(\'' + thiz.getView().getId() + '\').getController().scalemodeclick()" type="checkbox" ' + checked + '>'; },
            ...
            if (!initialized) {
              ...
              legend.innerHTML = this.getModel('legendModel').getData()[this._langu.toUpperCase()]; //document.querySelector('#legend-content').innerHTML;;
            ...
            chart.on('expcollclick', $.proxy(function (sender, collapse, id, ids) {
              if (!collapse) {
                var node = sender.getNode(id);
                if (node.parent) {
                  sender.expand(id, ids, $.proxy(function () {
                    if (this._level > 1) {
                      var _ids = [];
                      for (var i1 = 0; i1 < ids.length; i1++) {
                        _ids = _ids.concat(sender.getNode(ids[i1]).childrenIds);
                      }

                      sender.expand(ids[0], _ids, $.proxy(function () {
                        if (this._level > 2) {
                          var _ids = [];
                          for (var i1 = 0; i1 < ids.length; i1++) {
                            _ids = _ids.concat(sender.getNode(ids[i1]).childrenIds);
                          }
                          sender.expand(ids[1], _ids, $.proxy(function () {
                            var ids = [];
            ...
            chart.on('redraw', function (sender, args) {
              ...
              if (thiz.scaleMode && (sender.manager.action == OrgChart.action.expand || sender.manager.action == OrgChart.action.collapse)) {
                sender.fit();
              }
              ...
              if (thiz.isSidePanelOpen && sender.manager.action == OrgChart.action.update) {
            ...
            chart.editUI.on('show', function (sender, nodeId, args1, args2) {
              return true;
            });
            ...
            chart.on('searchclick', $.proxy(function (sender, nodeId) {
    ...
    onLang: function (oEvent) {
      ...
      legend.style.color = '#757575';
      legend.innerHTML = this.getModel('legendModel').getData()[this._langu.toUpperCase()];
      ...
      this.bus.publish("Detail", "refreshMasterPage", { "root": this._root, "date": this._date, "langu": this._langu });
    },

    onTheme: function (oEvent) {
      this.chart.config.template = oEvent.getParameter("selectedItem").getKey();
      if (this.chart.element.lastChild.children[0].id === 'legend-header') {
        this.chart.element.removeChild(this.chart.element.lastChild);
      }
    ...
    onDate: function (oEvent) {
      ...
      this.bus.publish("Detail", "refreshMasterPage", { "root": this._root, "date": this._date });
    ...
    _submitVariant: function () {
      ...
      this.getOwnerComponent().getModel().create("/sim_dheaderSet", sim_dheader, {
    ...
    orgChartTemplates: function (OrgChart) {
      ...
      */
      ...
      OrgChart.templates.customAna = Object.assign({}, OrgChart.templates.ana);
      ...
      OrgChart.templates.customAna.node = '<rect class="customAna outer" x="0" y="0" height="190" width="410" fill="#039BE5" stroke-width="1" stroke="#aeaeae" rx="7" ry="7"></rect>';


      OrgChart.templates.customAna.field_1 = '<text  style="font-size: 14px;" width="230" fill="#F57C00" x="110" y="70" text-overflow="multiline">{val}</text>';
      OrgChart.templates.customAna.field_2 = '<text style="font-size: 12px;" width="230" fill="#afafaf" x="110" y="85">{val}</text>';
      ...
      OrgChart.templates.cool.node = '<rect class="cool outer" filter="url(#cool-shadow)"  x="0" y="0" height="190" width="410" fill="#ffffff" stroke-width="2" stroke="#eeeeee" rx="10" ry="10"></rect>' +
      ...
      OrgChart.templates.cool.field_0 = '<foreignobject x="110" y="10" width="280" height="40"><p style="font-size:14px;color:gray;word-break: break-word;line-height:14px;margin:4px 0 0 0">{val}</p></foreignobject>';

      OrgChart.templates.cool.field_1 = '<text  style="font-size: 14px;" width="230" fill="#F57C00" x="110" y="70" text-overflow="multiline">{val}</text>';
      OrgChart.templates.cool.field_2 = '<text style="font-size: 12px;" width="230" fill="#afafaf" x="110" y="85">{val}</text>';
      ...
      OrgChart.templates.cool.node += '<foreignobject class="exp node" style="visibility: hidden;" x="370" y="15" width="40" height="40"><span class="icon-home-share" style="font-family: SAP-icons;font-size:1.5rem;">&#xe0e7;</span></i></foreignobject>';

      //Copy cool template before adding point4,5
      OrgChart.templates.coolbf = Object.assign({}, OrgChart.templates.cool);


      OrgChart.templates.cool.point_4 = '<text style="font-size: 20px;" fill="#F57C00" x="80" y="160" text-anchor="middle">{val}</text>';
      OrgChart.templates.cool.point_5 = '<text style="font-size: 20px;" fill="#F57C00" x="180" y="160" >{val}</text>';
      OrgChart.templates.cool.point_6 = '<text style="font-size: 20px;" fill="#F57C00" x="330" y="160" >{val}</text>';
      OrgChart.templates.cool.point_7 = '<text style="font-size: 20px;" fill="#F57C00" x="380" y="160" >{val}</text>';
      ...
      OrgChart.templates.coolbf.node = '<rect class="cool outer" filter="url(#cool-shadow)"  x="0" y="0" height="190" width="410" fill="#ffffff" stroke-width="1" stroke="#eeeeee" rx="10" ry="10"></rect>' +
        '<rect class="cool" fill="#ffffff" x="100" y="10" width="300" height="100" rx="10" ry="10" filter="url(#cool-shadow)"></rect>' +
      ...
      OrgChart.templates.coolbf.point_1 = '<text style="font-size: 16px;" fill="#F57C00" x="30" y="165" >{val}</text>';
      OrgChart.templates.coolbf.point_2 = '<text style="font-size: 16px;" fill="#F57C00" x="140" y="165" >{val}</text>';
      OrgChart.templates.coolbf.point_3 = '<text style="font-size: 16px;" fill="#F57C00" x="240" y="165" >{val}</text>';
      ...
      */
      ...
      OrgChart.templates.olivia.img_0 =
        `<clipPath id="{randId}" class="zolivia"><circle cx="50" cy="70" r="30"></circle></clipPath>
      ...
      OrgChart.templates.ula.field_2 =
        `<text data-width="145" data-text-overflow="multiline" style="font-size: 8px;font-weight: bold;" fill="#757575" x="80" y="82">{val}</text>`;

      OrgChart.templates.ula.img_0 =
        `<clipPath id="{randId}" class="zula"><circle cx="40" cy="60" r="30"></circle></clipPath>
      ...
      */
      ...
      OrgChart.templates.ana.node = '<rect class="zana outer" x="0" y="0" height="{h}" width="{w}" fill="#039BE5" stroke-width="1" stroke="#aeaeae" rx="5" ry="5"></rect>';
      ...
      OrgChart.templates.ana.node += '<foreignobject class="exp node" style="visibility: hidden;" x="100" y="10" width="40" height="40"><span class="icon-home-share" style="font-family: SAP-icons;font-size:1.5rem;">&#xe0e7;</span></i></foreignobject>';
      ...
      var _menu = {
        ...
        pdf: {
          ...
          onClick: $.proxy(function (nodeId) {
            ...
            this.chart.exportPDF({ filename: node.id + "_export.pdf", expandChildren: false, nodeId: nodeId });
        ...
        pdfExapnd: {
          ...
          onClick: $.proxy(function (nodeId) {
            this.exportflag = true;
            var node = this.chart.get(nodeId);
            this.chart.exportPDF({ filename: node.id + "_export.pdf", expandChildren: true, nodeId: nodeId });
        ...
        png: {
          ...
          onClick: $.proxy(function (nodeId) {
            this.exportflag = true;
            var node = this.chart.get(nodeId);
            this.chart.exportPNG({ filename: node.id + "_export.png", expandChildren: false, nodeId: nodeId });
          }, this)
        },
        pngExapnd: {
          ...
          onClick: $.proxy(function (nodeId) {
            this.exportflag = true;
            var node = this.chart.get(nodeId);
            this.chart.exportPDF({ filename: node.id + "_export.png", expandChildren: true, nodeId: nodeId });
        ...
        setAsRoot: {
          ...
          onClick: $.proxy(function (nodeId) {
            ...
            this.chart.expand(node.id, this.chart.getNode(nodeId).childrenIds, $.proxy(function () {
              this.chart.fit();
    ...
    getEditForm: function () {
      ...
      var _elements = [
        ...
        { type: 'select', options: [{ value: 'O', text: "Organizasyon Birimi" }, { value: 'S', text: 'Pozisyon' }, { value: 'P', text: 'Kişi' }], label: 'Tip', binding: 'O' },
        { type: 'textbox', label: 'Metin', binding: 'S', vlidators: { required: 'Is required' } },
    ...
    dateControl: function (oEvent) {
      ...
      this.dateControlInt = setInterval(this.dateControlInterval.bind(this), 30000);
    ...
    dateControlInterval: function () {
      ...
      this.getOwnerComponent().getModel().callFunction("/checkDateData", {
        urlParameters: _data,
        success: $.proxy(function (oData, oResponse) {
          ...
          if (oData.checkDateData && oData.checkDateData.Type === 'S') {
            ...
            var dialog = new Dialog({
              ...
              type: 'Message',
              content: new Text({
                text: oResourceBundle.getText("dateDateResult", [formatter.formatDate(this._date)])
